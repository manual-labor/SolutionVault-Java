...
public class Shortest_Distance_Dijkstra_1753 {
    ...
    static void dijkstra(int start) {
        ...
        distance[start] = 0;
        boolean[] isChecked = new boolean[vertexCount+1];

        while (!priorityQueue.isEmpty()) {
            Node currentNode = priorityQueue.poll();

            if (currentNode.cost > distance[currentNode.vertex])
                continue;

            if (!isChecked[currentNode.vertex]) {
                isChecked[currentNode.vertex] = true;
                for (Node node : graph.get(currentNode.vertex)) {
                    if (distance[node.vertex] > currentNode.cost + node.cost) {
                        ...
                        priorityQueue.offer(new Node(node.vertex, currentNode.cost + node.cost));
                    }
                }
            }
        }
