...
public class DiscountEvent_Implementation {
    public int solution(String[] want, int[] number, String[] discount) {
        ...
        for (Map.Entry<String, Integer> entry : wantMap.entrySet()) {
            int valueByDiscount = discountMap.getOrDefault(entry.getKey(), 0);
            if (entry.getValue() > valueByDiscount) {
                signUp = false;
                break;
            }
        }
        ...
        for (int i = 10; i < discount.length; i++) {
            ...
            for (Map.Entry<String, Integer> entry : wantMap.entrySet()) {
                int valueByDiscount = discountMap.getOrDefault(entry.getKey(), 0);
                if (valueByDiscount == 0 || entry.getValue() > valueByDiscount) {
                    signUp = false;
                    break;
                }
            }
